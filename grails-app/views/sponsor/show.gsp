	...
	<body>
		...
		<div id="show-sponsor" class="content scaffold-show" role="main">
			...
			<ol class="property-list sponsor">
				...
				<g:if test="${sponsorInstance?.sponsorship}">
				<li class="fieldcontain">
					<span id="sponsorship-label" class="property-label"><g:message code="sponsor.sponsorship.label" default="Sponsorship" /></span>
					
						<g:each in="${sponsorInstance.sponsorship}" var="s">
						<span class="property-value" aria-labelledby="sponsorship-label"><g:link controller="sponsorship" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
						</g:each>
